...
const CreateTask = async (req, res) => {
    try {
        ...
        if (!title || !status) {
            return res.status(400).json({ success: false, message: "Please provide title and status" });
        }
        ...
        if (sharedWith) {
            if (!Array.isArray(sharedWith)) {
                return res.status(400).json({ success: false, message: "sharedWith must be an array" });
            }

            sharedInfo = await Profile.find({ Email: { $in: sharedWith } }).select("_id");
            if (sharedInfo.length !== sharedWith.length) {
                return res.status(400).json({ success: false, message: "One or more sharedWith emails are invalid" });
            }
        }
...
const GetTasks = async (req, res) => {
    try {
        ...
        }).populate("owner", "Name").populate("sharedWith", "Name Email");
        ...
        return res.status(500).json({ success: false, message: "Failed to get task", success: false });
    }
};
...
const UpdateTask = async (req, res) => {
    try {
        ...
        const loggedInUser = req.user;
        const taskId = req.params.id;
        const { title, status, description, deadline, sharedWith } = req.body;
        if (!taskId) {
            return res.status(400).json({ success: false, message: "Please select task" });
        }
        ...
        if (sharedWith) {
            ...
            getTask.sharedWith = sharedInfo.map(user => user._id);
        }


        if (title) getTask.title = title;
        if (description) getTask.description = description;
        if(deadline) getTask.deadline = new Date(deadline).toISOString();
        if (status) {
            if (!["delayed", "completed", "running"].includes(status.toLowerCase())) {
                return res.status(400).json({success: false, message: "Invalid status" });
            }
        ...
        return res.status(201).json({ success: true, message: "Task updated successfully"});
    } catch (error) {
        return res.status(500).json({ success: false, message: "Updation Failed", error: error.message });
    }
